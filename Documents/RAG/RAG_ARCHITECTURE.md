...
- [Data Flow](#data-flow)
- [Two-Stage RAG Pipeline](#two-stage-rag-pipeline-retrieval--reranking)
...
## Two-Stage RAG Pipeline (Retrieval + Reranking)

RAG система использует двухэтапный пайплайн для поиска релевантных документов:

```
Запрос пользователя
        ↓
┌───────────────────────────────────────┐
│  1. RETRIEVAL (векторный поиск)       │
│                                       │
│  Модель: nomic-embed-text             │
│  Что делает: превращает текст в       │
│  вектор, ищет похожие по косинусу     │
│  Результат: Top-K документов          │
│  (быстро, но "тупо" - только лексика) │
└───────────────────────────────────────┘
        ↓
┌───────────────────────────────────────┐
│  2. RERANKING (переранжирование)      │
│                                       │
│  Модель: qwen2.5-coder:14b (LLM)      │
│  Что делает: LLM оценивает каждый     │
│  документ на релевантность запросу    │
│  Результат: отсортировано по смыслу   │
│  (медленно, но понимает контекст)     │
└───────────────────────────────────────┘
        ↓
   Финальный ответ
```
...
| Характеристика | Retrieval (Bi-Encoder) | Reranking (Cross-Encoder) |
|----------------|------------------------|---------------------------|
| Модель | `nomic-embed-text` | `qwen2.5-coder:14b` |
| Скорость | Быстро (~50ms на 1000 docs) | Медленно (~200ms на 10 docs) |
| Понимание контекста | Низкое (лексическое) | Высокое (семантическое) |
| Масштабируемость | Отлично | Плохо |
| Роль | Грубый отбор кандидатов | Точная сортировка |

### Пример работы

**Запрос:** "Хочу посмотреть на Эйфелеву башню"

| Этап | Модель | Результат |
|------|--------|-----------|
| 1. Retrieval | `nomic-embed-text` | paris-france: 0.85, paris-texas: 0.83 |
| 2. Reranking | `qwen2.5-coder` | paris-france: 9/10, paris-texas: 4/10 |

**Объяснение:**
- На первом этапе оба документа получают высокий скор, т.к. оба содержат "Эйфелева башня"
- На втором этапе LLM понимает контекст "посмотреть" и выбирает оригинальную башню в Париже, а не копию в Техасе

### Настройки реранкинга

```
Реранкинг результатов:
├── Использовать реранкинг: ВКЛ
├── Стратегия: Cross-Encoder (LLM)
├── Провайдер: Ollama (Local)
├── Модель: qwen2.5-coder:14b
└── Мин. скор релевантности: 5 (из 10)

Общие параметры поиска:
├── Top-K результатов: 10
└── Минимальный скор: 0.3
```

### Важные замечания

- `nomic-embed-text` — **embedding модель**, используется ТОЛЬКО для первого этапа (Retrieval)
- Для реранкинга нужна **генеративная LLM** (qwen2.5-coder, llama3, mistral и т.д.)
- Низкие пороги на первом этапе (minScore: 0.3) позволяют реранкеру увидеть больше кандидатов
