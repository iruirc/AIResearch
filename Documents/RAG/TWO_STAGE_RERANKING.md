...
## Модели для Retrieval vs Reranking

RAG система использует **разные типы моделей** для каждого этапа:
...
│  Тип: Embedding model (Bi-Encoder)    │
...
│  Тип: Generative LLM (Cross-Encoder)  │
...
| **Тип модели** | Embedding | Generative LLM |
...
- `nomic-embed-text` — **embedding модель**, используется ТОЛЬКО для Retrieval
- Для Reranking нужна **генеративная LLM**:
  - `qwen2.5-coder:14b` ✅
  - `llama3:8b` ✅
  - `mistral:7b` ✅
  - `nomic-embed-text` ❌ (не подходит!)
